...
- `GET /api/icons`: lista de IDs soportados.
- `GET /api/svgs`: devuelve todos los SVG embebidos (endpoint pesado).
  - Por defecto devuelve `404`.
  - Si configuras el secret/binding `SVGS_API_KEY`, requiere `x-api-key: <SVGS_API_KEY>` (o `?key=...`).

# CI/CD

- CI en PR: corre `format:check`, `lint` y `test`.
- Preview deploy (PR): añade el label `preview` a un PR para desplegar un Worker temporal con nombre `skill-icons-pr-<num>`.
  - Secrets requeridos en GitHub: `CF_API_TOKEN` (y normalmente `CF_ACCOUNT_ID`).
- Dependabot: actualiza dependencias (npm) y Actions semanalmente.
- Supply chain: los workflows pinean actions por SHA.

# Security

Ver [SECURITY.md](SECURITY.md) para reporte responsable de vulnerabilidades.
